...
-- Which Store has historically brought the most revenue?
 ...
 SELECT EXTRACT(YEAR FROM RENTAL_DATE),EXTRACT(MONTH FROM RENTAL_DATE)AS MONTH_, COUNT(RENTAl_ID) AS NUMBER_RENTAL
 FROM RENTAL
...
AVG(RENTAL_RATE) AS AVERAGE_RANTEL
FROM FILM
...
WHEN LENGTH > 90 THEN 'OVER 1.5 HRD'
ELSE 'ERROR'
...
SELECT AC.ACTOR_ID,AC.FIRST_NAME,LAST_NAME,COUNT(FA.FILM_ID) AS NUMBER_OF_MOVIES
